...
- [x] **Add `[tool.ruff]` config** — shipped with the pyflakes `F` ruleset (target py311) + `ruff` in the dev extra. Auto-cleaned 30 safe findings (23 unused imports, 6 redundant test re-imports, 1 unused test var, 1 f-string); reverted 1 (`timeclock.live_cache`, kept via `# noqa: F401` — the reconciliation tests patch it through the module). No CI/pre-push hook (would block the frequent pushes against pre-existing style debt). **⚠ Flagged for review (not fixed):** ruff `F601` found a duplicate `"department"` key in `routes/settings.py:259` — `eff["department"]` silently overrides the earlier `loc.department`; likely latent, removing the dead key is behavior-identical. **✅ Shipped 2026-06-03.**
