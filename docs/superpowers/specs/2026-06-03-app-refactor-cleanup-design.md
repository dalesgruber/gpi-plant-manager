...
- [x] **Dedupe pure helpers:** `_fmt_hf` → shared `time_format.fmt_decimal_hour`, and `_parse_time` (`work_schedule_store` now imports `schedule_store`'s) — **✅ both shipped 2026-06-03**. `person_id_to_name()` (was rebuilt at 5 sites) → one `attendance.person_id_to_name(name_to_id=None)` helper, no extra queries — **✅ shipped 2026-06-03**. `unwrap_m2o` → one `odoo_client.unwrap_m2o()` (11 inline sites + `time_off_sync._unwrap_many2one`); left `odoo_sync._m2o_id` (distinct None-returning contract) — **✅ shipped 2026-06-03**. *(All dedupes done.)*
- [x] **Add `[tool.ruff]` config** — shipped with the pyflakes `F` ruleset (target py311) + `ruff` in the dev extra. Auto-cleaned 30 safe findings (23 unused imports, 6 redundant test re-imports, 1 unused test var, 1 f-string); reverted 1 (`timeclock.live_cache`, kept via `# noqa: F401` — the reconciliation tests patch it through the module). No CI/pre-push hook (would block the frequent pushes against pre-existing style debt). **F601 resolved:** removed the dead duplicate `"department"` key in `routes/settings.py` (kept `eff["department"]`, behavior-identical); `ruff check` is now clean across the repo. **✅ Shipped 2026-06-03.**
...
- [x] **Consolidate `settings_store` KV access** → new `app_settings.get_setting`/`set_setting` own the decode + `::jsonb` upsert; `settings_store` (incl. goal-target reads, int-coercion preserved) + `odoo_sync._write_last_sync` delegate. Left `odoo_sync._read_last_sync` (scalar-string decode the generic helper can't safely handle). Goal coercion verified preserved. **✅ Shipped 2026-06-03.**

---

## Tier 2 — Bigger refactors · own mini-plan + characterization tests first

- [x] **Decompose `routes/staffing.py` (1284)** → `staffing_attendance.py` (6 I/O helpers, lazy imports + degrade-to-empty preserved) + pure `staffing_view.build_staffing_bays()` (render model, +16 characterization tests). `_LATE_REPORT_CACHE`/`_bust_after_mutation`/`/api/late-report` stayed in the route. Byte-identical (final review APPROVE), 561 passed. Plan: `docs/superpowers/plans/2026-06-03-staffing-py-decomposition.md`. **✅ Shipped 2026-06-03.**
- [x] **Decompose `routes/timeclock_time_off.py` (1310)** → pure `time_off_wizard.py` (validators) + `time_off_calendar.py` (label/holiday/month-grid/fan-out); `_approved_by_day` kept as thin I/O wrapper (monkeypatch preserved), validators re-imported under `_`-aliases. +39 unit tests; `build_calendar_grid` verified byte-equivalent by harness; 600 passed. **Deferred:** `time_off_store`/leave-types-upsert dedup (Medium risk, low value — separate change). Plan: `docs/superpowers/plans/2026-06-03-timeclock-time-off-decomposition.md`. **✅ Shipped 2026-06-03.**
- [x] **Lift recycling goal-math out of `routes/departments.py` (953)** → new pure `recycling_data.py` (`progress_color`, `compute_per_wc_expected` goal denominator, aggregation helpers); characterization tests added FIRST (`tests/test_recycling_goal_math.py`) pinning the June regressions; deduped the inline-assign popover + absent-names; fixed stale `expected_by_wc` docstring. `station_target`/`productive_minutes_in_window` stay injected so the goal can't drift. Behavior byte-identical (final review APPROVE), +18 tests, 545 passed. Plan: `docs/superpowers/plans/2026-06-03-recycling-goal-math-decomposition.md`. **✅ Shipped 2026-06-03.**
