...
def test_adjusted_downtime_event_during_break_excluded(_lunch_1130_to_1200):
    ...
    Setup: machine reports Stopped at 11:25 for 50 minutes (event ends
    12:15). Samples bracket lunch within TRANSFER_GAP (active interval
    spans the break). Without the break-subtraction fix, all 50 minutes
    would count. With the fix, the 30 min of lunch is subtracted → 20m.
    """
    samples = [
        (_utc(11, 20), 1),  # last pallet before lunch
        (_utc(12, 15), 1),  # first pallet after lunch (45 min gap, < 60min)
    ]
    downtime_rows = [(_utc(11, 25), 50)]  # Stopped 11:25 -> 12:15
...
def test_adjusted_downtime_event_fully_outside_break(_lunch_1130_to_1200):
    ...
    downtime_rows = [(_utc(9, 10), 15)]  # Stopped 9:10 -> 9:25
